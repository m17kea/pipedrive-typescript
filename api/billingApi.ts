...
import { SubscriptionAddonsResponse200 } from '../model/subscriptionAddonsResponse200';
...
export class BillingApi {
     ...
     */
    public async getCompanyAddons (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: SubscriptionAddonsResponse200;  }> {
        const localVarPath = this.basePath + '/billing/subscriptions/addons';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: SubscriptionAddonsResponse200;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "SubscriptionAddonsResponse200");
