...
import { GetUserSettingsResponse200 } from '../model/getUserSettingsResponse200';
...
export class UserSettingsApi {
     ...
     */
    public async getUserSettings (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetUserSettingsResponse200;  }> {
        const localVarPath = this.basePath + '/userSettings';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetUserSettingsResponse200;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetUserSettingsResponse200");
